...
def convert_weights(model, load_path, n_classes, n_layers=34, save_path=None, dataset='sports1m'):
    ...
    r += [
        ['conv1_', 'r21dv10_init_'], ['_w', '_weight'], ['_rm', '_running_mean'],
        ['_riv', '_running_var'], ['_spatbn', '_conv'],
    ...
    if n_layers == 34:
        ...
        r += [
            ['shortcut_projection_0_conv_', 'r21dv10_stage1_block1_down_'],
            ['shortcut_projection_3_conv_', 'r21dv10_stage2_block1_down_'],
    ...
    r += [['_conv_relu_', '_'], ['_dense0_beta', '_dense0_bias']]
    ...
    for k in pdict.keys():
        if k in comp:
            print(
                k + ' :: ' + str(pdict[k].data().shape) + ' <--loaded from-- ' + comp[k] + ' :: ' +
                str(x['blobs'][comp[k]].shape)
            )
            found.append(k)
        else:
...
def transform_frames(frames, length_rgb=8, scale_h=128, scale_w=171, crop=112, bgr=True):
    ...
    for i in range(length_rgb):
        ...
        frames[i] = cv2.resize(
            frames[i], (scale_w, scale_h), fx=0, fy=0, interpolation=cv2.INTER_AREA
        )  # resize
        if bgr:
    ...
    for i in range(length_rgb):
        frames[0][i] = mx.nd.image.normalize(
            frames[0][i], mean=[0.43216, 0.394666, 0.37645], std=[0.22803, 0.22145, 0.216989]
        )
