...
flags.DEFINE_integer('epochs', 40, 'How many training epochs to complete')
flags.DEFINE_integer('num_hidden', 128, 'Dimension of the states')
flags.DEFINE_integer('emb_size', 100, 'Dimension of the embedding vectors')
flags.DEFINE_float('dropout', 0.2, 'dropout applied to layers (0 = no dropout)')
flags.DEFINE_integer('num_layers', 2, 'Number of layers in the encoder  and decoder')
flags.DEFINE_integer(
    'num_bi_layers', 1, 'Number of bidirectional layers in the encoder and decoder'
)

flags.DEFINE_string('cell_type', 'gru', 'gru or lstm')
flags.DEFINE_integer(
    'batch_size', 128, 'Batch size for detection: higher faster, but more memory intensive.'
)

flags.DEFINE_integer('beam_size', 4, 'Beam size.')

flags.DEFINE_float('lp_alpha', 1.0, 'Alpha used in calculating the length penalty')
flags.DEFINE_integer('lp_k', 5, 'K used in calculating the length penalty')
flags.DEFINE_integer('test_batch_size', 32, 'Test batch size')
flags.DEFINE_integer('num_buckets', 5, 'Bucket number')

flags.DEFINE_string(
    'bucket_scheme', 'constant', 'Strategy for generating bucket keys. It supports: '
...
flags.DEFINE_float('bucket_ratio', 0.0, 'Ratio for increasing the throughput of the bucketing')
flags.DEFINE_integer('tgt_max_len', 50, 'Maximum length of the target sentence')
flags.DEFINE_string('optimizer', 'adam', 'optimization algorithm')
flags.DEFINE_float('lr', 1E-3, 'Initial learning rate')
flags.DEFINE_float('lr_update_factor', 0.5, 'Learning rate decay factor')
...
flags.DEFINE_string('backbone', 'DenseNet121', 'Backbone CNN name')
flags.DEFINE_string(
...
flags.DEFINE_integer(
    'data_shape', 512, 'The width and height for the input image to be cropped to.'
)
flags.DEFINE_integer('every', 1, 'Use only every this many frames: [train, val, test] splits')
flags.DEFINE_string('feats_model', None, 'load CNN features as npy files from this model')
flags.DEFINE_string(
    'emb_file', 'embeddings-ex.txt', 'the word embedding file generated by train_embeddings.py'
)
...
def main(_argv):
    ...
    write_sentences(
        val_tgt_sentences,
        os.path.join('models', 'captioning', 'experiments', FLAGS.model_id, 'val_gt.txt')
    )
    write_sentences(
        test_tgt_sentences,
        os.path.join('models', 'captioning', 'experiments', FLAGS.model_id, 'test_gt.txt')
    )
    ...
    if os.path.exists(os.path.join('models', 'captioning', 'experiments', FLAGS.model_id)):
        ...
        if len(files) > 0:
            ...
            model.load_parameters(
                os.path.join('models', 'captioning', 'experiments', FLAGS.model_id, model_name),
                ctx=ctx
            )
            logging.info(
    ...
    logging.info(
        'Use beam_size={}, alpha={}, K={}'.format(FLAGS.beam_size, FLAGS.lp_alpha, FLAGS.lp_k)
    )
    ...
    # run the training
    train(
        data_train, data_val, data_test, model, loss_function, val_tgt_sentences,
...
def evaluate(data_loader, model, loss_function, translator, data_train, ctx):
    ...
    for batch_id, (src_seq, tgt_seq, src_valid_length, tgt_valid_length,
                   inst_ids) in enumerate(data_loader):
        ...
        for i in range(max_score_sample.shape[0]):
            translation_out.append(
                [
                    data_train.vocab.idx_to_token[ele]
...
def train(
    ...
    for epoch_id in range(start_epoch, FLAGS.epochs):
        ...
        for batch_id, (src_seq, tgt_seq, src_valid_length,
            ...
            if (batch_id + 1) % FLAGS.log_interval == 0:
                ...
                logging.info(
                    ...
                    .format(
                        epoch_id, batch_id + 1,
                        len(train_data_loader), log_avg_loss / FLAGS.log_interval,
                        np.exp(log_avg_loss / FLAGS.log_interval), wps / 1000, log_wc / 1000
                    )
                )
                log_start_time = time.time()
                log_avg_loss = 0
        ...
        valid_loss, valid_translation_out = evaluate(
            val_data_loader, model, loss_function, translator, data_train, ctx
        )
        ...
        test_loss, test_translation_out = evaluate(
            test_data_loader, model, loss_function, translator, data_train, ctx
        )
        ...
        if valid_bleu_score > best_valid_bleu:
            ...
            save_path = os.path.join(
                'models', 'captioning', 'experiments', FLAGS.model_id, 'valid_best.params'
            )
        ...
        model.save_parameters(
            os.path.join(
                'models', 'captioning', 'experiments', FLAGS.model_id,
    ...
    write_sentences(
        valid_translation_out,
        os.path.join('models', 'captioning', 'experiments', FLAGS.model_id, 'best_valid_out.txt')
    )
    write_sentences(
        test_translation_out,
        os.path.join('models', 'captioning', 'experiments', FLAGS.model_id, 'best_test_out.txt')
    )
