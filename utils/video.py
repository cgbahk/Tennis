    ...
    while frame < end:  # lets loop through the frames until the end
        ...
        if frame % every == 0:  # if this is a frame we want to write out based on the 'every' argument
            ...
            save_path = os.path.join(
                frames_dir, video_filename, "{:010d}".format(start), "{:010d}.jpg".format(frame)
            )
            if not os.path.exists(
                    save_path) or overwrite:  # if it doesn't exist or we want to overwrite anyways
...
def video_to_frames(video_path, frames_dir, overwrite=False, every=1, chunk_size=1000):
    ...
    if total < 1:  # if video has no frames, might be and opencv error
        print(
            "Video has no frames. Check your OpenCV + ffmpeg installation, can't read videos!!!\n"
    ...
    for frame_chunk in frame_chunks:
        ...
        os.makedirs(
            os.path.join(frames_dir, video_filename, "{:010d}".format(frame_chunk[0])),
            exist_ok=True
        )
...
def frames_to_video(frames_dir, video_path, fps=30):
    ...
    video = cv2.VideoWriter(
        video_path, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), fps, (width, height)
    )
