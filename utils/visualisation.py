...
COLOURS = (
    (148, 148, 148), (255, 176, 143), (214, 127, 235), (143, 195, 255), (142, 235, 164),
    (255, 243, 140), (255, 214, 148), (235, 131, 154), (162, 147, 255), (145, 235,
                                                                         223), (208, 255, 145)
)
...
def visualise_events(dataset, results, video_path, gt=None, max=-1):
    ...
    video = cv2.VideoWriter(
        video_path, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), 30,
    ...
    for i, path in tqdm(enumerate(order), total=len(order), desc='Generating Vis'):
        ...
        canvas[:height, :, :] = img
        canvas[height:height + banner_height, :width - 1, :] = canvas[height:height + banner_height,
                                                                      1:, :]
        ...
        canvas[height + banner_height - bar_height - gt_height:height + banner_height - gt_height,
               width - 1:width, :] = COLOURS[ind]

        if gt:
            gtp = gt[path]
            canvas[height + banner_height - int(gt_height * .9):height + banner_height -
                   int(gt_height * .3), width - 1:width, :] = COLOURS[gtp]

        for c, cls in enumerate(classes):
            if ind == c:
                if gt is not None:
                    if ind == gtp:
                        canvas[height + banner_height + 2:,
                               c * cls_width:(c + 1) * cls_width, :] = (0, 255, 0)
                    else:
                        canvas[height + banner_height + 2:,
                               c * cls_width:(c + 1) * cls_width, :] = (0, 0, 255)
                else:
                    canvas[height + banner_height + 2:,
                           c * cls_width:(c + 1) * cls_width, :] = (0, 0, 0)

                canvas[height + banner_height + 2 + pred_border:-pred_border, c * cls_width +
                       pred_border:(c + 1) * cls_width - pred_border, :] = COLOURS[c]
            else:
                canvas[height + banner_height + 2:,
                       c * cls_width:(c + 1) * cls_width, :] = COLOURS[c]
            canvas = cv2.putText(
                canvas, cls,
